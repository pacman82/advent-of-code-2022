...
fn main() -> Result<(), Error> {
    ...
    let sum = count_redundant(input);
    println!("{sum}");
    Ok(())
}

fn count_redundant(input: impl BufRead) -> u64 {
    let mut count = 0;
...
mod tests {
    ...
    fn assignment_pairs() {
        let redundant = |bytes| {
            AssignmentPair::extract_from_bytes(bytes)
                .unwrap()
                .redundant()
        };
        assert!(!redundant(b"2-4,6-8"));
        assert!(redundant(b"2-8,3-7"));
    }
}
